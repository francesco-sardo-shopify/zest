...
export function TopBar({ currentPage, bookId }: TopBarProps) {
  const isLibraryPage = currentPage === "library";

  return (
    <header className="bg-yellow-500 shadow-md">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        {/* Logo and App Name with optional back chevron */}
        <div className="flex items-center space-x-2">
          {!isLibraryPage && (
            <Link
              to="/"
              className="mr-1 text-white hover:text-yellow-100"
          ...
          <Link to="/" className="flex items-center space-x-2">
            <div className="w-8 h-8 text-yellow-200 flex items-center justify-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
                className="w-6 h-6"
              >
                <path d="M12 2c-5.5 0-10 4.5-10 10s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-2 15l-5-5 1.4-1.4L9 13.2l7.6-7.6L18 7l-8 10z" />
              </svg>
            </div>
